...
\changed{
	{\bf Thread-aware policies.}
	Many recent OLTP kernels use a decentralized architecture~\cite{lim2017cicada, tu2013speedy}, where there is no centralized validator.  
	...
	In such architectures, while batching operations across transactions does not apply, we can reorder the transactions to reduce inter-thread conflicts and to achieve better data locality as a side-effect. Intuitively, for a batch of incoming transactions, we schedule the transactions that access the same objects (i.e., \emph{conflicting} transactions) to execute on the same thread to reduce conflicts across threads. On the same thread, conflicting transactions are executed serially and thus do not conflict. Because conflicting transactions access the same objects, reordering also improves caching.
	
	We adapt our reordering algorithm for multi-threaded synchronous architecture with a thread-aware policy, which schedules transactions thread by thread. We initialize the weight of the transaction to be the number of read-write conflicts it has with a batch of incoming transactions. For each thread, we first pick the transaction with the most conflicts from the remaining transactions. Iteratively, the policy updates the weight of each remaining transaction to the number of transactions it conflicts with within this thread and picks the next transaction with the highest weight, breaking ties by the number of objects a transaction shares with the transactions already scheduled to this thread. This policy greedily tries to put conflicting transactions on the same thread, and it tries to minimize conflicts across threads. As we will see later in our experiments (see Section \ref{sec:OtherOLTP}), this simple policy significantly improves throughput and latency and reduces tail latencies.
}% end changed
