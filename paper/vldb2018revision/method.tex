...
\changed{
	...
	In such an architecture, while batching operations across transactions does not apply, we can assign transactions to threads to reduce inter-thread conflicts, and as a side-effect, achieve better caching. Intuitively, for a batch of incoming transactions, we schedule conflicting transactions to execute on the same thread where they are processed serially. Because conflicting transactions access the same objects, reordering also improves caching within a thread.
	
	We extend our sort-based algorithm to a \emph{thread-aware policy} for a multi-threaded decentralized OLTP architecture which schedules transactions thread by thread. We batch a number of transactions and schedule the same number of transactions on each thread.\footnote{More sophisticated assignments are possible, but we leave them for future work.} At each thread, we assign weights to transactions first based on the number of conflicts to transactions in the batch that have already been scheduled to this thread, then the number of object accesses shared between this transaction and the scheduled transactions, and finally its number of conflicts to the unscheduled transactions in this batch. For each thread, our sort-based algorithm will first pick the transaction with the most conflicts against the remaining transactions, and then it iteratively updates the weights of the transactions left to pick the next one with the highest weight. This policy greedily puts conflicting transactions on the same thread to minimize conflicts across threads; in general, it tries to schedule transactions accessing the same objects to the same thread. As we will see later in our experiments (Section \ref{sec:OtherOLTP}), this simple policy significantly improves throughput and reduces tail latencies.
}% end changed
