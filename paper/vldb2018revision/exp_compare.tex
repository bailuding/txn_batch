...
As the degree of data contention increases, either with more threads or a more skewed data distribution, reordering outperforms the other systems with higher throughput and lower tail latencies. With 28 threads, reordering improves throughput by up to 5.0x and reduces the 99\% latency by up to 91\%.
}

\changed{
In the next experiment, we configure our strategy with a batch size of $4\times num\_threads$ and schedule $4$ transactions to each thread within a batch after reordering, i.e., a transaction is never scheduled across batch boundaries and will at most get delayed within a small window. The reordering is 5x faster than processing a transaction.\footnote{The reordering can be further parallelized as we discuss in Appendix \ref{sec:sec:experiments:parallel}}.
