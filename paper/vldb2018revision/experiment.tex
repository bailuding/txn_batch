...
\item How does storage and validator batching and reordering
  affect system performance? (Section~\ref{subsec:experiment:batching})
\vspace{-0.5em}
\item What is the impact of different policies for alternative performance goals? (Section~\ref{subsec:validator_reordering:policy})
\vspace{-0.5em}
\item How do we incorporate our techniques on top of a state-of-the-art OLTP system and a commercial DBMS, and what is the resulting performance? (Section~\ref{sec:OtherOLTP} and~\ref{subsec:experiment:compare})
\end{enumerate}
Table~\ref{tbl:exp} summarizes the trade-offs of parameters evaluated. Appendix \ref{sec:experiments:parallel} evaluates the parallelization of our algorithms.
...
\changed{
	...
	\begin{tabular}{|c|c|}
		...
		\hline
		batch size & Larger batch size has more flexibility of reordering but increases transaction latency. \\
		\hline
		FVS algorithms & Sort-based algorithm reorders faster at the cost of excluding more transactions from FVS. \\
		\hline
		...
		\hline
		policy: thread-aware & Reorder transactions to minimize conflicts between threads and increase cache locality.\\
		\hline
	\end{tabular}
	\label{tbl:exp}
}
...
\changed{We first describe the experimental setup for our research system. Section \ref{subsec:experiment:compare} describes experiments with a commercial database system and Section \ref{sec:OtherOLTP} describes experiments with an open-source OLTP sytem. 
	}
...
\changed{(4)} In the Small Bank benchmark, batching and reordering improves the throughput by up to a factor of 3.1x, and it reduces the average and percentile latency by up to 68\% and 62\%.
%\vspace{-0.5em}
%\item 

\changed{(5) Integrating our techniques to a high performance OLTP system (Cicada) shows that, compared with the-state-of-the-art OLTP systems, the thread-aware reordering policy improves transaction throughput by a factor of up to 5.0x and reduces the 99\% latency by a factor of up to 11x under a write-intensive workload.}
