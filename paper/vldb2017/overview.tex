...
In validator batching, we buffers and batches transaction validation requests at the validator as they arrive.
We can first make a conceptual distinction between two types of aborts: intra-batch and inter-batch aborts. If $T(i)$ and $T(j)$ are in the same batch, we call the resulting abort of $T(j)$ an \emph{intra-batch abort}, otherwise, we call the resulting abort of $T(j)$ an \emph{inter-batch abort}.
