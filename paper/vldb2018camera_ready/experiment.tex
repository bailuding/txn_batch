...
\cut{In our evaluation, we study the performance of our validator reordering algorithms (Section~\ref{sec:exp_algorithms}), the end-to-end impact of batching and reordering (Section~\ref{subsec:experiment:batching}) with different policies (Section~\ref{subsec:experiment:policy}), and the impact of parallelization at the validator (Appendix~\ref{sec:experiments:parallel}). We also evaluate our techniques on an open source OLTP system (Section~\ref{sec:exp:cicada}) and a commercial DBMS (Section~\ref{subsec:experiment:compare}). Section~\ref{sec:exp:summary} summarizes all the results.}
\cut{
...
% Parallelization.
We parallelize the transaction generation with two transaction clients.\cut{ the storage, and the transaction reordering at the validator. By default, two transaction clients populate transactions concurrently to supply sufficient load.} We use two storage workers concurrently process reads and writes, 
and the writes are applied based on its data versioning as described in Section~\ref{subsec:storage_reordering}. 
We further parallelize \cut{the components in} the validator as described in Appendix~\ref{sec:parallel}.
...
We implement our prototype in Java. All the experiments run on a machine with
...
%\vspace{-.5em}
\begin{enumerate}[leftmargin=*, nolistsep]
\item The sort-based greedy algorithm finds accurate FVS and reduces the running time of SCC-based greedy algorithms by $74\%$, resulting in the best system performance. 
...
\cut{Batching by itself already improves throughput significantly, and adding reordering on top of it consistently improves throughput \cut{by up to $1.3\times$}. In addition, reordering significantly reduces the average and tail latency by up to $25\%$ and $70\%$ respectively.} While it is always beneficial to use storage reordering, validator reordering consistently improves tail latency but can hurt throughput and average latency under extremely high data contention.
%\vspace{-0.5em}
%\item 
...
\item When incorporated into a commercial DBMS, on Small Bank benchmark, our techniques improve the throughput by up to $3.1\times$, and it reduces the average and percentile latency by up to $68\%$ and $62\%$.
%\vspace{-0.5em}
%\item 
...
\item Integrated to a high performance OCC-based OLTP system, our techniques with thread-aware reordering policy improves the throughput by up to $2.2\times$ and reduces the tail latency by up to $71\%$ compared with other state-of-the-art systems under write-intensive workload.

\item Integrated to a commercial DBMS-X, our techniques increase the throughput by up to $2.8\times$ and reduce the average latency by up to $66\%$ under high data contention. This shows that there is much room for performance improvements using our techniques even for mature DBMSs.
\end{enumerate}  
