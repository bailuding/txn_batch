...
Intel Xeon E5-2630 CPU @2.20GHz and 16GB RAM. All our experimental figures show the averages of 10 runs, each lasting for 60 seconds in between a 10-second warm-up and a 10-second cool-down. The standard deviation is not significant in any of the experiments, so we omit the error bars for clarity of presentation. We report \changed{throughput} (the number of committed transactions per second), average and percentile latency.
...
%\vspace{-.5em}
\begin{enumerate}[leftmargin=*, nolistsep]
\item The sort-based greedy algorithm finds accurate FVS and reduces the running time of SCC-based greedy algorithms by $74\%$, resulting in the best end-to-end system performance among all the algorithms. 
...
%\item 
\cut{
\item When integrated into a commercial DBMS, on Small Bank benchmark, our techniques improve the throughput by up to $3.1\times$, and it reduces the average and percentile latency by up to $68\%$ and $62\%$ respectively.}
%\vspace{-0.5em}
%\item 

\item For alternative reordering policies at the validator, prioritizing transactions with a combination of the degree of a transaction in the dependency graph and its number of restarts reduces tail latency by up to 82\%, without sacrificing either the throughput or average latency.
%\vspace{-0.5em}
%\item 


\item Integrated to a high performance OCC-based OLTP system, our techniques with thread-aware reordering policy improves the throughput by up to $2.2\times$ and reduces the tail latency by up to $71\%$ compared with other state-of-the-art OLTP systems under write-intensive workload.
