...
the validator checks if there are conflicts with previously committed transactions. \eat{, and makes a``success'' or ``failure'' validation decision.}
...
\cut{
	...
	%\item 
	$\bullet$ In a detailed experimental study of the impact of storage and validator batching in a prototype system, as well as on top of a commercial database system, we show that batching and reordering always increases transaction throughput and, surprisingly, also reduces transaction latency, especially the tail latencies. For workloads with high data contention, batching and reordering can improve the throughput by up to 3.1x and can reduce the average transaction latency by up to 82\% for a micro benchmark and the Small Bank Benchmark~\cite{alomari2008icde} (Section \ref{sec:experiments}).
	%\end{itemize}
}

\changed{
	\item In a detailed experimental study of a prototype system, as well as on top of a state-of-the-art OLTP system and a commercial database, we show that batching and reordering always increases transaction throughput and, surprisingly, also reduces transaction latency, especially tail latencies. For workloads with high data contention, batching and reordering can improve the throughput by up to $2.2\times$ and reduce tail latencies by up to $71\%$ compared with the state-of-the-art OLTP systems (Section \ref{sec:experiments}).\cut{We further show how to reduce the amortized overhead of reordering through parallelism (Appendix~\ref{sec:parallel} and~\ref{sec:experiments:parallel}).}
}
