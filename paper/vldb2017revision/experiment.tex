...
\vspace{-.8em}
\item As we increase the level of parallelism in validator reordering, both the throughput and the latency improve, especially when the data contention is medium to high. With four transaction reordering workers, the throughput increases by up to 2.6x, the average latency reduces by up to 39\%, and the tail latency reduces by up to 41\% as compared to that with a single reordering worker.
\vspace{-.8em}
\item Batching by itself improves throughput significantly. Adding reordering on top of batching consistently improves throughput, and significantly reduces the average and the percentile latency. With batching and reordering, the throughput increases by up to 2.7x, the average latency reduces by up to 67\%, and the percentile latency reduces by up to 82\% as compared to the baseline. In addition, it is always beneficial to use storage reordering. Validator reordering consistently improves the percentile transaction latency but can hurt the throughput and latency when the data contention is at the extremes.
\vspace{-.8em}
