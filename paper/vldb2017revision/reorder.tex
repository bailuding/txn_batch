...
If $G$ is acyclic, then there exists a commit order $Q$ on $G$ that respects all read-write dependencies. We can construct $Q$ by repeatedly committing a transaction whose corresponding node in $G$ has no outgoing edge using topology sort. If $G$ is not acyclic, we can show that the problem is NP-hard by reducing the NP-hard problem of finding a directed feedback vertex set to it~\cite{karp1972reducibility}. 
...
% TODO(blding): fix the description, replace |B| with |V| and |E|, fix C, use partition algorithm.
...
again (line 14). We repeat this procedure until the graph is empty (line 4). The
time complexity of this algorithm is $O(C|B|)$, where $|B|$ is the size of the batch, and $C$ is a constant for sorting, updating the graph, and the reduction of iterations using the multi-factor. Since the nodes are mostly sorted after updating the graph, the sorting in each iteration (except for the first one) is mostly linear. In practice, this algorithm is faster but less accurate than the SCC-based greedy algorithm. However, as we will see in Section~\ref{sec:experiments}, it produces results of comparable accuracy to the SCC-based algorithm in practice.
