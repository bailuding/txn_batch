...
Since the reordering sits at the critical path of the transaction execution, it can increase the latency of a transaction, resulting in higher chance of conflicts. 

In addition to reduce the overhead of the reordering algorithm, we further parallelize the reordering so that its amortized overhead decreases.

In centralized reordering, validator takes a batch of transactions, reorders them, and then validates the transactions and caches the updates of committed ones.
This centralized component can be decomposed into four subcomponents: batch preparation, transaction reordering, transaction validation, and cache update. In the batch preparation component, transactions are organized as batches upon arriving at validator. Each batch of transactions are then reordered to minimize the conflicts within a batch. A batch of transactions is validated against previously committed updates. Finally, the updates from committed transactions in the batch are applied to the validator cache in their serialization order.

% Pipeline parallelism
We can first introduce pipeline parallelism into validator. Each of the four subcomponents can be operated on a different thread. The batch preparation subcomponent receives requests from the transaction coordinator, packages transaction into batches, and sends a reordering request to the transaction reordering subcomponent. Next, the transaction reordering subcomponent first pre-validates a batch of transactions based on the latest database snapshot available, then reorders the remaining transactions, and sends a validation request consisting of the remaining transactions to the transaction validation subcomponent. 
...
In the pipelined reordering, the validation is decoupled into two phases. Let's denote a database snapshot as $S_k$, where transactions whose timestamps are less or equal to $k$ has been committed in this snapshot. In the reordering subcomponent, a batch of transactions $B$ is validated against the latest database snapshot $S_m$ available. The remaining transactions after pre-filtering is reordered as a batch $B'$ and sent to the validation subcomponent. $B'$ is again validated against the current database snapshot $S_n$. Since the validation subcomponent can be validating uncommitted transactions while $B$ is being processed at the reordering subcomponent, when $B$ arrives at the validation subcomponent, the snapshot of the database can be advanced and $n\geq m$. Thus, transactions in $B'$ can still abort due to conflicts with previously committed transactions in $S_n$.

% Parallelism in each subcomponent.
We can further introduce parallelism into subcomponents. In batch preparation, multiple threads can package transactions into batches. We can either assign each transaction coordinator sends its validation request to a specific batch preparation thread, or we can create a consumer-producer queue to connect the coordinators and the batch preparation threads. In transaction reordering, multiple threads can consume reordering requests from the batch preparation threads, and reordering batches of transactions concurrently. Since the transactions are not serialized yet in the reordering stage, the threads can send the processed batches to the validation subcomponent in any order. At the validation subcomponent, transactions are serialized and validated against all previously committed transactions. We cannot simply validate each individual transaction on different threads since transactions come from different batches can conflict with each other. However, within an ordered batch, since the only source of conflicts is from all the transactions committed prior the batch, the validation of each transaction within the batch can be processed in parallel. Since conflicts can happen across batch boundaries, processing a new batch should only start after all the threads finish processing the previous batch. The transactions are first serialized at the batch level. Within each ordered batch, the transactions are serialized by the ordering produced from the reorder subcomponent, after removing the transactions that conflict against the current database snapshot at the validation subcomponent. The cache update subcomponent applies the updates in serialized order. While its operation is light-weight, if needed, we can introduce partitioned parallelism to update the cache concurrently. The idea is to partition the key space and break a transaction into smaller pieces, and one thread is assigned to apply the updates to one database partition. 

% Figure
Figure~\ref{fig:reorder:validator} shows the architecture of parallel validator. It has pipeline parallelism with the four subcomponents: batch preparation, transaction reordering, transaction validation, and cache update. The requests within each subcomponent can be processed concurrently. 

\begin{figure}[t]
	\centering
	\includegraphics[width=1\columnwidth]{./figures/validator}
	\vspace{-1em}
	\caption{The architecture of parallel validator. It is decoupled into four subcomponents for pipeline parallelism: batch preparation, transaction reordering, transaction validation, and cache update. Each subcomponent can be further parallelized. The reordering subcomponent pre-validates transactions in a batch against the latest database snapshot available. The validation subcomponent validates transactions concurrently within the boundary of an ordered batch. The cache update subcomponet applies the update from an ordered and validated batch to create a new database snapshot.}
	\vspace{-1em}
